...
public class SetupCMD implements CommandExecutor {
    ...
    private UltraSkyWarsSetup setup;

    public SetupCMD(UltraSkyWars plugin, UltraSkyWarsSetup setup) {
        this.plugin = plugin;
        this.setup = setup;
    }
        ...
        if (sender instanceof Player) {
            ...
            switch (args[0].toLowerCase()) {
                ...
                case "delete": {
                    ...
                    if (!plugin.getGm().getWorlds().contains(delete)) {
                ...
                case "clone": {
                    ...
                    if (!plugin.getGm().getWorlds().contains(arena1)) {
                        p.sendMessage("§cThis game not exists.");
                        return true;
                    }
                    String arena2 = args[2];
                    if (plugin.getGm().getWorlds().contains(arena2)) {
                        p.sendMessage(plugin.getLang().get("setup.arena.alreadyMapEdit"));
                        return true;
                    }
