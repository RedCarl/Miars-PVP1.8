...
public class WorldListener implements Listener {
    ...
    public WorldListener(UltraSkyWars plugin) {
        this.plugin = plugin;
    }

    @EventHandler(priority = EventPriority.MONITOR)
    public void onWeather(WeatherChangeEvent e) {
        if (plugin.getGm().getWorlds().contains(e.getWorld().getName())) {
    ...
    public void onSpawn(CreatureSpawnEvent e) {
        ...
        if (plugin.getGm().getWorlds().contains(e.getLocation().getWorld().getName())) {
            e.setCancelled(true);
        }
    }
