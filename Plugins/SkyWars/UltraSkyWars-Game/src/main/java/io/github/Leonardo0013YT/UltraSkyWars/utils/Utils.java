...
import io.github.Leonardo0013YT.UltraSkyWars.objects.Multiplier;
...
public class Utils {
    ...
    private static UltraSkyWars plugin = UltraSkyWars.get();
    ...
    public static Giant spawn(Location ballon, Location fence, ItemStack item) {
        ...
        if (fence != null) {
            ...
            asl.setPassenger(s);
            plugin.getVc().getReflection().freezeMob(s);
    ...
    public static void updateSB() {
        if (!plugin.isStop()) {
            Bukkit.getOnlinePlayers().stream().filter(p -> !plugin.getGm().isPlayerInGame(p)).forEach(p -> plugin.getSb().update(p, null));
        }
    }
    ...
    public static void updateSB(Game game) {
        if (!plugin.isStop()) {
            ...
            game.getCached().forEach(p -> plugin.getSb().update(p, ge));
        }
    }
