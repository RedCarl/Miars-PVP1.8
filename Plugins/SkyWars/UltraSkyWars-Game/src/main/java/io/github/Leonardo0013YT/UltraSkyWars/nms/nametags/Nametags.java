...
import io.github.Leonardo0013YT.UltraSkyWars.nms.NMSReflection;
...
public class Nametags {
    ...
    static NametagVersion version;

    static {
        try {
            version = NametagVersion.valueOf(Bukkit.getServer().getClass().getPackage().getName().split("\\.")[3]);
    ...
    public Nametags(String teamName, String displayName, String prefix) {
        try {
            ...
            this.setField(version.getB(), XMaterial.isNewVersion() ? ChatComponentText.newInstance(displayName) : displayName);
            this.setField(version.getA(), teamName);
            this.setField(version.getC(), XMaterial.isNewVersion() ? ChatComponentText.newInstance(prefix) : prefix);
    ...
    private void remove(String teamName, Player p) {
        try {
            ...
            NMSReflection.sendPacket(p, packet);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    ...
    public void sendToPlayer(Player pl) {
        NMSReflection.sendPacket(pl, packet);
    }
