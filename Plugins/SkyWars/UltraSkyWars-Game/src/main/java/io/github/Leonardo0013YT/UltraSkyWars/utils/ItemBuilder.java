...
public class ItemBuilder {
    ...
    public static ItemStack parseVariables(Player p, ItemStack item) {
        ...
        for (int k = 0; k < lore.size(); k++) {
            ...
            lore.set(k, UltraSkyWars.get().getAdm().parsePlaceholders(p, value));
        }
        im.setLore(lore);
        i.setItemMeta(im);
        return i;
    ...
    public static ItemStack parseVariables(Player p, ItemStack item, String[]... t) {
        ItemStack i = item.clone();
        if (!i.hasItemMeta() || !i.getItemMeta().hasDisplayName()) {
            return i;
        }
            ...
            for (String[] s : t) {
                ...
                for (int k = 0; k < lore.size(); k++) {
                    ...
                    if (value.contains(s1)) {
                        ...
                        lore.set(k, UltraSkyWars.get().getAdm().parsePlaceholders(p, newValue));
                    } else {
                        lore.set(k, UltraSkyWars.get().getAdm().parsePlaceholders(p, value));
                    }
                }
    ...
    public static ItemStack parseChestVariables(Player p, ItemStack item, CallBackAPI<String> selected, String[]... t) {
        ItemStack i = item.clone();
        if (!i.hasItemMeta() || !i.getItemMeta().hasDisplayName()) {
            return i;
        }
