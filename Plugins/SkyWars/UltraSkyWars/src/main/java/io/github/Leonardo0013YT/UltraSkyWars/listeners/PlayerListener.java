...
public class PlayerListener implements Listener {
    ...
    public void onDeath(PlayerDeathEvent e) {
        ...
        plugin.getVc().getReflection().sendActionBar(plugin.getLang().get("action.kill").replaceAll("<players>", "" + (game.getPlayers().size())), game.getCached());
    ...
    public void onDismount(EntityDismountEvent e) {
        if (e.getEntity() instanceof Player && e.getDismounted().hasMetadata("NO_TARGET")) {
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask(UltraSkyWars.get(), () -> {
