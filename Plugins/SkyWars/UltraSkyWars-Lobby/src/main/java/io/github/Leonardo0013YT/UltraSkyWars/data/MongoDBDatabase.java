...
public class MongoDBDatabase implements Database {
    ...
    private static HashMap<UUID, SWPlayer> SWPlayers = new HashMap<>();
    private MongoClient mc;
    private DB admin;
    private DBCollection players, multipliers;
    ...
    public void loadTopElo() {
        ...
        for (DBObject m : multipliers.find().sort(new BasicDBObject("elo", -1)).limit(10)) {
    ...
    public void loadTopCoins() {
        int pos = 1;
        List<String> tops = new ArrayList<>();
        for (DBObject m : multipliers.find().sort(new BasicDBObject("coins", -1)).limit(10)) {
    ...
    public void loadTopKills() {
        int pos = 1;
        List<String> tops = new ArrayList<>();
        for (DBObject m : multipliers.find().sort(new BasicDBObject("kills", -1)).limit(10)) {
    ...
    public void loadTopWins() {
        int pos = 1;
        List<String> tops = new ArrayList<>();
        for (DBObject m : multipliers.find().sort(new BasicDBObject("wins", -1)).limit(10)) {
    ...
    public void loadTopDeaths() {
        int pos = 1;
        List<String> tops = new ArrayList<>();
        for (DBObject m : multipliers.find().sort(new BasicDBObject("deaths", -1)).limit(10)) {
