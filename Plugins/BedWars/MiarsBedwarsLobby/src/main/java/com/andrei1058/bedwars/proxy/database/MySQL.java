...
public class MySQL implements Database{
    ...
    public boolean hasStats(UUID uuid) {
        ...
        try (ResultSet rs = connection.createStatement().executeQuery("SELECT id FROM bedwars_global_stats WHERE uuid = '" + uuid.toString() + "';")) {
            result =  rs.next();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void init() {
        ...
        try (Statement s = connection.createStatement()) {
            s.executeUpdate("CREATE TABLE IF NOT EXISTS bedwars_quick_buy (id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, uuid VARCHAR(200), " +
        ...
        try (Statement s = connection.createStatement()) {
            s.executeUpdate("CREATE TABLE IF NOT EXISTS bedwars_player_levels (id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, uuid VARCHAR(200), " +
        ...
        try (Statement s = connection.createStatement()) {
            s.executeUpdate("CREATE TABLE IF NOT EXISTS bedwars_player_language (id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, uuid VARCHAR(200), " +
                    "iso VARCHAR(200));");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void updateLocalCache(UUID uuid) {
        if (!isConnected()) connect();
        try (ResultSet rs = connection.createStatement().executeQuery("SELECT * FROM bedwars_global_stats WHERE uuid = '" + uuid.toString() + "';")) {
            if (rs.next()) {
    ...
    public Object[] getLevelData(UUID player) {
        ...
        try (ResultSet rs = connection.prepareStatement("SELECT level, xp, name, next_cost FROM bedwars_player_levels WHERE uuid = '" + player.toString() + "';").executeQuery()) {
            if (rs.next()) {
                r[0] = rs.getInt("level");
                r[1] = rs.getInt("xp");
    ...
    public void setLevelData(UUID player, int level, int xp, String displayName, int nextCost) {
        if (!isConnected()) connect();
        try  (ResultSet rs = connection.prepareStatement("SELECT id from bedwars_player_levels WHERE uuid = '" + player.toString() + "';").executeQuery()) {
            if (!rs.next()) {
                PreparedStatement ps = connection.prepareStatement("INSERT INTO bedwars_player_levels VALUES (?, ?, ?, ?, ?, ?);");
                ps.setInt(1, 0);
                ...
                if (displayName == null) {
                    ps = connection.prepareStatement("UPDATE bedwars_player_levels SET level=?, xp=? WHERE uuid = '" + player.toString() + "';");
                } else {
                    ps = connection.prepareStatement("UPDATE bedwars_player_levels SET level=?, xp=?, name=?, next_cost=? WHERE uuid = '" + player.toString() + "';");
                }
                ps.setInt(1, level);
                ps.setInt(2, xp);
                if (displayName != null) {
    ...
    public void setLanguage(UUID player, String iso) {
        if (!isConnected()) connect();
        try (ResultSet rs = connection.createStatement().executeQuery("SELECT iso FROM bedwars_player_language WHERE uuid = '" + player.toString() + "';")) {
            Statement s = connection.createStatement();
            if (rs.next()) {
                s.executeUpdate("UPDATE bedwars_player_language SET iso='" + iso + "' WHERE uuid = '" + player.toString() + "';");
            } else {
                s.executeUpdate("INSERT INTO bedwars_player_language VALUES (0, '" + player.toString() + "', '" + iso + "');");
            }
            s.close();
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public void ping() {
        try (Statement s = connection.createStatement()) {
            s.execute("SELECT id FROM bedwars_player_levels WHERE id=0;");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
