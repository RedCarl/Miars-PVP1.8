...
public class Arena implements IArena {
    ...
    public Arena(String name, Player p) {
        ...
        if (!BedWars.getAPI().getRestoreAdapter().isWorld(name)) {
            if (p != null) {
                p.sendMessage(ChatColor.RED + "There isn't any map called " + name);
            }
    ...
    public boolean addPlayer(Player p, boolean skipOwnerCheck) {
        ...
        if (status == GameState.waiting || (status == GameState.starting && (startingTask != null && startingTask.getCountdown() > 1))) {
            ...
            for (Player on : players) {
                ...
                on.sendMessage(
                        SupportPAPI.getSupportPAPI().replace(on,getMsg(on, Messages.COMMAND_JOIN_PLAYER_JOIN_MSG))
                                ...
                                .replace("{miarsSuffix}", ColorParser.parse(mRank.getSuffix()))
                                .replace("{miarsPrefix}", ColorParser.parse(mRank.getPrefix()))
                                .replace("{miarsNameColor}", ColorParser.parse(mRank.getNameColor()))
                );
            }
...
//            SidebarService.getInstance().giveSidebar(p, this, false);
            ...
            String iso = getPlayerLanguage(p).getIso();
    ...
    public void removePlayer(@NotNull Player p, boolean disconnect) {
        ...
        if (status == GameState.starting && players.size() < minPlayers) {
                    ...
                    if (!team.isBedDestroyed()) {
                        for (Player p2 : this.getPlayers()) {
                            p2.sendMessage(getMsg(p2, Messages.TEAM_ELIMINATED_CHAT).replace("{TeamColor}", team.getColor().chat().toString())
                                    .replace("{TeamName}", team.getDisplayName(getPlayerLanguage(p2))));
                        }
            ...
            if (team != null) {
                ...
                if (lastDamager != null && isPlayer(lastDamager) && killerTeam != null) {
                    ...
                    PlayerKillEvent event = new PlayerKillEvent(this, p, lastDamager, player -> getMsg(player, message), cause);
        ...
        for (Player on : getSpectators()) {
            ...
            MPlayer mPlayer = MPlayerDataStorage.getInstance().getMPlayer(p);
            MRank mRank = MRankDataStorage.getInstance().getMRank(mPlayer.getRank());
            on.sendMessage(SupportPAPI.getSupportPAPI().replace(on,getMsg(on, Messages.COMMAND_LEAVE_MSG))
                    .replace("{vPrefix}", getChatSupport().getPrefix(p))
                    .replace("{playername}", p.getName())
                    .replace("{player}", p.getDisplayName())
                    .replace("{miarsSuffix}", ColorParser.parse(mRank.getSuffix()))
...
//        SidebarService.getInstance().giveSidebar(p, this, true);
    ...
    public boolean isBlockPlaced(Block block) {
        for (org.bukkit.util.Vector v : getPlaced()) {
            if (v.getX() == block.getX() && v.getY() == block.getY() && v.getZ() == block.getZ()) {
                return true;
            }
        }
        return false;
...
//        spectators.forEach(c -> SidebarService.getInstance().giveSidebar(c, this, false));
    ...
    public synchronized void refreshSigns() {
        for (Block b : getSigns()) {
            ...
            if (!(b.getType().toString().endsWith("_SIGN") || b.getType().toString().endsWith("_WALL_SIGN"))) {
                continue;
            }
    ...
    public static void sendLobbyCommandItems(Player p) {
        if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_LOBBY_ITEMS_PATH) == null) {
            return;
        }
    ...
    public ITeam getPlayerTeam(String playerCache) {
        for (ITeam t : getTeams()) {
            for (Player p : t.getMembersCache()) {
                if (p.getName().equals(playerCache)) {
                    return t;
                }
            }
        }
                ...
                if (winner != null) {
                    ...
                    for (Player p : world.getPlayers()) {
                        p.sendMessage(getMsg(p, Messages.GAME_END_TEAM_WON_CHAT).replace("{TeamColor}", winner.getColor().chat().toString())
                                .replace("{TeamName}", winner.getDisplayName(getPlayerLanguage(p))));
                        ...
                        for (String s : getList(p, Messages.GAME_END_TOP_PLAYER_CHAT)) {
                            String message = s.replace("{firstName}", firstName.isEmpty() ? getMsg(p, Messages.MEANING_NOBODY) : firstName).replace("{firstKills}", String.valueOf(first))
                                    ...
                                    .replace("{TeamColor}", winner.getColor().chat().toString()).replace("{TeamName}", winner.getDisplayName(getPlayerLanguage(p)));
    ...
    public static boolean canAutoScale(String arenaName) {
        ...
        for (IArena ar : Arena.getArenas()) {
            if (ar.getArenaName().equalsIgnoreCase(arenaName)) {
                ...
                if (ar.getStatus() == GameState.waiting || ar.getStatus() == GameState.starting) {
                    return false;
                }
            }
            // count active clones
