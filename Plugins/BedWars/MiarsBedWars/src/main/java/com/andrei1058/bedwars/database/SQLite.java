...
public class SQLite implements Database {
    ...
    public void init() {
        ...
        try {
            ...
            sql = "CREATE TABLE IF NOT EXISTS bedwars_global_stats (id INTEGER PRIMARY KEY AUTOINCREMENT, " +
            ...
            try (Statement st = connection.createStatement()) {
                sql = "CREATE TABLE IF NOT EXISTS bedwars_quick_buy_2 (uuid VARCHAR(36) PRIMARY KEY, " +
            ...
            try (Statement st = connection.createStatement()) {
                sql = "CREATE TABLE IF NOT EXISTS bedwars_player_levels (id INTEGER PRIMARY KEY AUTOINCREMENT, uuid VARCHAR(200), " +
                        "level INTEGER, xp INTEGER, name VARCHAR(200), next_cost INTEGER);";
                st.executeUpdate(sql);
            }
            try (Statement st = connection.createStatement()) {
                sql = "CREATE TABLE IF NOT EXISTS  bedwars_player_language (id INTEGER PRIMARY KEY AUTOINCREMENT, uuid VARCHAR(200), " +
                        "iso VARCHAR(200));";
                st.executeUpdate(sql);
            }
    ...
    public boolean hasStats(UUID uuid) {
        String sql = "SELECT uuid FROM bedwars_global_stats WHERE uuid = ?;";
        try {
            checkConnection();
    ...
    public void saveStats(PlayerStats stats) {
        ...
        try {
            ...
            if (hasStats(stats.getUuid())) {
                sql = "UPDATE bedwars_global_stats SET last_play=?, wins=?, kills=?, final_kills=?, looses=?, deaths=?, final_deaths=?, beds_destroyed=?, games_played=?, name=? WHERE uuid = ?;";
                try (PreparedStatement statement = connection.prepareStatement(sql)) {
                ...
                sql = "INSERT INTO bedwars_global_stats (name, uuid, first_play, last_play, wins, kills, final_kills, looses, deaths, final_deaths, beds_destroyed, games_played) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
                try (PreparedStatement statement = connection.prepareStatement(sql)) {
    ...
    public PlayerStats fetchStats(UUID uuid) {
        ...
        String sql = "SELECT * FROM bedwars_global_stats WHERE uuid = ?;";
        try {
            checkConnection();
    ...
    public void setQuickBuySlot(UUID p, String shopPath, int slot) {
        try {
            ...
            try (PreparedStatement statement = connection.prepareStatement("SELECT uuid FROM bedwars_quick_buy_2 WHERE uuid = ?;")) {
                statement.setString(1, p.toString());
                try (ResultSet rs = statement.executeQuery()) {
                    if (!rs.next()) {
                        try (PreparedStatement ps = connection.prepareStatement("INSERT INTO bedwars_quick_buy_2 (uuid, slot_19, slot_20, slot_21, slot_22, slot_23, slot_24, slot_25, slot_28, slot_29, slot_30, slot_31, slot_32, slot_33, slot_34, slot_37, slot_38, slot_39, slot_40, slot_41, slot_42, slot_43) VALUES(?,' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ');")) {
                            ps.setString(1, p.toString());
                            ps.execute();
                        }
                    }
                    BedWars.debug("UPDATE SET SLOT " + slot + " identifier " + shopPath);
                    try (PreparedStatement ps = connection.prepareStatement("UPDATE bedwars_quick_buy_2 SET slot_" + slot + " = ? WHERE uuid = ?;")) {
    ...
    public String getQuickBuySlots(UUID p, int slot) {
        ...
        try {
            ...
            try (PreparedStatement ps = connection.prepareStatement("SELECT slot_" + slot + " FROM bedwars_quick_buy_2 WHERE uuid = ?;")) {
                ps.setString(1, p.toString());
    ...
    public boolean hasQuickBuy(UUID uuid) {
        try {
            ...
            try (Statement statement = connection.createStatement()) {
                try (ResultSet rs = statement.executeQuery("SELECT uuid FROM bedwars_quick_buy_2 WHERE uuid = '" + uuid.toString() + "';")) {
                    if (rs.next()) {
                        rs.close();
                        return true;
                    }
    ...
    public Object[] getLevelData(UUID player) {
        ...
        try {
            ...
            try (PreparedStatement ps = connection.prepareStatement("SELECT level, xp, name, next_cost FROM bedwars_player_levels WHERE uuid = ?;")) {
    ...
    public void setLevelData(UUID player, int level, int xp, String displayName, int nextCost) {
        try {
            ...
            try (PreparedStatement pss = connection.prepareStatement("SELECT uuid from bedwars_player_levels WHERE uuid = ?;")) {
                pss.setString(1, player.toString());
                try (ResultSet rs = pss.executeQuery()) {
                    if (!rs.next()) {
                        try (PreparedStatement ps = connection.prepareStatement("INSERT INTO bedwars_player_levels (uuid, level, xp, name, next_cost) VALUES (?, ?, ?, ?, ?);")) {
                            //ps.setInt(1, 0);
                            ps.setString(1, player.toString());
                            ps.setInt(2, level);
                            ps.setInt(3, xp);
                        ...
                        try (PreparedStatement ps = displayName == null ? connection.prepareStatement("UPDATE bedwars_player_levels SET level=?, xp=? WHERE uuid = '" + player.toString() + "';") : connection.prepareStatement("UPDATE bedwars_player_levels SET level=?, xp=?, name=?, next_cost=? WHERE uuid = '" + player.toString() + "';")) {
                            ps.setInt(1, level);
                            ps.setInt(2, xp);
                            if (displayName != null) {
    ...
    public void setLanguage(UUID player, String iso) {
        try {
            ...
            try (Statement statement = connection.createStatement()) {
                try (ResultSet rs = statement.executeQuery("SELECT iso FROM bedwars_player_language WHERE uuid = '" + player.toString() + "';")) {
                    if (rs.next()) {
                        try (Statement st = connection.createStatement()) {
                            st.executeUpdate("UPDATE bedwars_player_language SET iso='" + iso + "' WHERE uuid = '" + player.toString() + "';");
                        }
                    } else {
                        try (PreparedStatement st = connection.prepareStatement("INSERT INTO bedwars_player_language (uuid, iso) VALUES (?, ?);")) {
    ...
    public String getLanguage(UUID player) {
        ...
        try {
            ...
            try (PreparedStatement ps = connection.prepareStatement("SELECT iso FROM bedwars_player_language WHERE uuid = ?;")) {
                ps.setString(1, player.toString());
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
    ...
    public void pushQuickBuyChanges(HashMap<Integer, String> updateSlots, UUID uuid, List<QuickBuyElement> elements) {
        ...
        String sql = hasQuick ? "UPDATE bedwars_quick_buy_2 SET " + columns + " WHERE uuid=?;" : "INSERT INTO bedwars_quick_buy_2 (uuid," + columns + ") VALUES (?," + values + ");";
        try {
            checkConnection();
    ...
    public HashMap<Integer, String> getQuickBuySlots(UUID uuid, int[] slot) {
        ...
        try {
            ...
            try (PreparedStatement ps = connection.prepareStatement("SELECT * FROM bedwars_quick_buy_2 WHERE uuid = ?;")) {
