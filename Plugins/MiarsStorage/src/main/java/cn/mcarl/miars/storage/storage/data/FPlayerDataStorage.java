...
public class FPlayerDataStorage {
    ...
    public void putFPlayer(FPlayer fPlayer){
        try {
            ...
            FPlayer data = MiarsStorage.getMySQLStorage().queryFPlayerDataByUUID(fPlayer.getUuid());
            dataMap.put(
                    fPlayer.getUuid(),
                    data
            );
    ...
    public FPlayer getFPlayer(Player player){
        ...
        if (data==null){
            try {
                // 初始化玩家数据
                putFPlayer(new FPlayer(player.getUniqueId().toString(),0L,0L,0L,null,new Date(System.currentTimeMillis())));
