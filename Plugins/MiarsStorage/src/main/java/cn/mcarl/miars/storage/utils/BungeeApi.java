...
public class BungeeApi {
  ...
  /**
   * Get or create new BungeeApi instance
   *
   * @param plugin the plugin instance.
   * @return the BungeeApi instance for the {@code plugin}
   * @throws NullPointerException if the {@code plugin} is {@code null}
   */
  public synchronized static BungeeApi of(Plugin plugin) {
  ...
  public void connect(Player player, String serverName) {
    ...
    player.sendMessage(ColorParser.parse("&fConnecting to &b"+serverName+" &fServer..."));
  }
  ...
  private void onPluginMessageReceived(String channel, Player player, byte[] message) {
    ...
    synchronized (callbackMap) {
      ...
      if (subchannel.equals("PlayerCount") || subchannel.equals("PlayerList") ||
        ...
        try {
          switch (subchannel) {
            case "PlayerCount" -> ((CompletableFuture<Integer>) callback).complete(input.readInt());
            case "PlayerList" ->
                    ((CompletableFuture<List<String>>) callback).complete(Arrays.asList(input.readUTF().split(", ")));
            case "UUIDOther" -> ((CompletableFuture<String>) callback).complete(input.readUTF());
            case "ServerIP" -> {
      ...
      try {
        switch (subchannel) {
          case "GetServers" ->
                  ((CompletableFuture<List<String>>) callback).complete(Arrays.asList(input.readUTF().split(", ")));
          case "GetServer", "UUID" -> ((CompletableFuture<String>) callback).complete(input.readUTF());
          case "IP" -> {
            String ip = input.readUTF();
