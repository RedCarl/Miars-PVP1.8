...
public class TablistAnimationTask extends BukkitRunnable {
    ...
    private int currentHeaderPause, currentFooterPause;
    private int currentHeaderDelay, currentFooterDelay, currentBodyDelay;
    private int currentHeader, currentFooter;
    ...
    public void run() {
        ...
        if (currentHeaderDelay != -1) {
            currentHeaderDelay++;
        }
        if (currentBodyDelay != -1) {
            currentBodyDelay++;
        }
        if (currentFooterDelay != -1) {
            currentFooterDelay++;
        }
        ...
        footer: {
            if (animation.isAnimateFooter()) {
                if (currentFooterDelay == animation.getFooterInterval()) {
                    ...
                    if(currentFooter == -1) {
                        if(currentFooterPause >= animation.getFooterAnimationDelay()) {
