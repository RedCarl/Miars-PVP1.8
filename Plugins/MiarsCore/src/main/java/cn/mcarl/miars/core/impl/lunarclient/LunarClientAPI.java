...
import cn.mcarl.miars.core.impl.lunarclient.event.LCPacketReceivedEvent;
import cn.mcarl.miars.core.impl.lunarclient.event.LCPacketSentEvent;
import cn.mcarl.miars.core.impl.lunarclient.event.LCPlayerUnregisterEvent;
import cn.mcarl.miars.core.impl.lunarclient.listener.LunarClientLoginListener;
import cn.mcarl.miars.core.impl.lunarclient.nethandler.LCPacket;
import cn.mcarl.miars.core.impl.lunarclient.nethandler.client.*;
...
import cn.mcarl.miars.core.impl.lunarclient.object.LCWaypoint;
import cn.mcarl.miars.core.impl.lunarclient.object.StaffModule;
...
public final class LunarClientAPI implements Listener {
    ...
    public void onEnable() {
        ...
        MiarsCore.getInstance().getServer().getPluginManager().registerEvents(new LunarClientLoginListener(this), MiarsCore.getInstance());
    }
    ...
    private void registerPluginChannel(String bukkitChannel) {
        Messenger messenger = MiarsCore.getInstance().getServer().getMessenger();
        messenger.registerOutgoingPluginChannel(MiarsCore.getInstance(), bukkitChannel);
        messenger.registerIncomingPluginChannel(MiarsCore.getInstance(), bukkitChannel, (channel, player, bytes) -> {
    ...
    public void unregisterPlayer(Player player, boolean quit) {
        ...
        if (quit) {
            ...
            playersNotRegistered.add(player.getUniqueId());
            MiarsCore.getInstance().getServer().getPluginManager().callEvent(new LCPlayerUnregisterEvent(player));
        }
    }
    ...
    public boolean sendPacket(Player player, LCPacket packet) {
        ...
        if (isRunningLunarClient(playerId)) {
            ...
            player.sendPluginMessage(MiarsCore.getInstance(), channel, LCPacket.getPacketData(packet));
